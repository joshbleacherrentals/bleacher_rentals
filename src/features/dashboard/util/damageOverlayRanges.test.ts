...
describe("computeDamageOverlayRanges", () => {
  ...
  it("ends the day before the maintenance event starts", () => {
    const ranges = computeDamageOverlayRanges([report()], [{ eventStart: "2025-03-16" }], DATES);
  ...
  it("uses the earliest maintenance event starting on or after the report", () => {
    const ranges = computeDamageOverlayRanges(
      [report()],
      [{ eventStart: "2025-03-18" }, { eventStart: "2025-03-15" }, { eventStart: "2025-03-11" }],
      DATES,
    );
  ...
  it("runs to the end of the grid when maintenance starts past the window", () => {
    const ranges = computeDamageOverlayRanges([report()], [{ eventStart: "2025-04-02" }], DATES);
  ...
  it("ignores resolved reports", () => {
    expect(
      computeDamageOverlayRanges([report({ resolvedAt: "2025-03-13T00:00:00Z" })], [], DATES),
    ).toEqual([]);
  });

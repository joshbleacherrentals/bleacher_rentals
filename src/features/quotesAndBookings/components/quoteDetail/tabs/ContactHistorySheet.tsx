...
import {
  fetchContactEvents,
  type ContactEvent,
  type ContactEventBuckets,
} from "../../../db/fetchContactEvents";
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col items-end gap-1 shrink-0">
        ...
        <span className="text-[10px] text-gray-400 capitalize">{event.relation}</span>
      </div>
    </a>
  );
}

function EventList({ events, emptyLabel }: { events: ContactEvent[]; emptyLabel: string }) {
  if (events.length === 0) {
  ...
  return (
    <div className="flex flex-col gap-1">
      {events.map((event) => (
        <EventRow key={`${event.id}-${event.relation}`} event={event} />
      ))}
    </div>
  );
}
...
export function ContactHistorySheet({
  ...
  const [buckets, setBuckets] = useState<ContactEventBuckets | null>(null);
    ...
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      <div className="fixed inset-y-0 right-0 flex w-full flex-col bg-white shadow-2xl ring-1 ring-black/10 animate-in slide-in-from-right sm:max-w-md">
        <div className="flex items-start justify-between border-b border-gray-100 px-6 py-4">
          <div>
            <h2 className="text-base font-semibold text-darkBlue">{contactName}</h2>
            <p className="mt-0.5 text-xs text-gray-500">Past and future events for this contact</p>
          </div>
          <button
            onClick={close}
